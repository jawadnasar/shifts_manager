    ...
    <div class="container-fluid pt-4 px-4">
        <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
            <div>
                ...
                <p class="text-muted small mb-0">Money received; receipt issued to payer or company.</p>
            </div>
        ...
        <div class="bg-light rounded p-4 mb-3">
            <form method="GET" action="{{ route('admin.receipts.index') }}">
                <div class="row g-3 align-items-end">
                    ...
                    <div class="col-md-2">
                        <label class="form-label" for="receipt_transid">Trans ID</label>
                        <input type="text" name="receipt_transid" id="receipt_transid" class="form-control"
                            value="{{ request('receipt_transid') }}" placeholder="Exact ID">
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2 d-flex gap-2">
                        ...
                        <a href="{{ route('admin.receipts.index') }}" class="btn btn-outline-secondary">Clear</a>
                    </div>
                </div>
            </form>
        </div>
                        ...
                        @forelse ($data as $row)
                            <tr>
                                ...
                                <td class="text-nowrap">
                                    <a href="{{ route('admin.receipts.show', $row) }}"
                                        class="btn btn-sm btn-outline-secondary">View</a>
                                    @if (Auth::user()->pri_editreceipt && $row->canEdit())
                                        <a href="{{ route('admin.receipts.edit', $row) }}"
                                            class="btn btn-sm btn-outline-primary">Edit</a>
                                    @endif
                                    @if (Auth::user()->pri_editreceipt && $row->canDelete())
                                        <form action="{{ route('admin.receipts.destroy', $row) }}" method="POST"
                                            class="d-inline"
